<?php
class ControllerModuleCustomMenuContent extends Controller {
    private $error = array(); // This is used to set the errors, if any.
 
    public function index() {
         // Loading the language file of custom_menu_content
        $this->load->language('module/custom_menu_content'); 
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('module/custom_menu_content', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );
          
        $data['action'] = $this->url->link('module/custom_menu_content', 'token=' . $this->session->data['token'], 'SSL'); // URL to be directed when the save button is pressed
        ...
        $this->load->model('module/custom_menu_content');
        $data['categories'] = array();
        $data['top_categories'] = array();

        $categories = $this->model_module_custom_menu_content->getCategories_custom(0);
        ...
        $this->response->setOutput($this->load->view('module/custom_menu_content.tpl', $data));
    ...
    protected function validate() {
        ...
        if (!$this->user->hasPermission('modify', 'module/custom_menu_content')) {
